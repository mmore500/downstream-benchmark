...
for attempt in {1..5}; do
    ...
    && python3.10 -m uv pip install -r \
        "${BATCHDIR_JOBSOURCE}/requirements.txt" \
...
algorithms = [
    ("steady", dstream.steady_algo.assign_storage_site_batched),
    ("stretched", dstream.stretched_algo.assign_storage_site_batched),
    ("tilted", dstream.tilted_algo.assign_storage_site_batched)
]
